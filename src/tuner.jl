...
```
    update!(x::Tuner, ac::Bool)

Update the tuner `x` with acceptance status `ac`. It simply counts the number of acceptances in the current block. If the end of the
...
```
function update!(x::Tuner, ac::Bool)
    x.ac_cnt += ac
    
    b = x.blk_size
    cb = x.cur_blk
    
    if x.cur_blk <= x.n_blk_max
        if x.cur_iter == b ## update tuning parameter every b iterations
            x.blk_acr[cb] = x.ac_cnt/b ## acceptance rate of last b iteration
